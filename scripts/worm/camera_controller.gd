...
func set_target(target: Node2D, lerp: bool = true):
	_target = target
	if is_inside_tree() and lerp:
		...
		_move_tween.tween_method(_move_tween_to_target.bind(global_position), 0.0, 1.0, lerp_duration).set_ease(Tween.EASE_IN_OUT).set_trans(Tween.TRANS_CUBIC)
